...
# -------------------------------
# Journalist: Edit Rejected Article
# -------------------------------
@login_required
def edit_article(request, pk):
    """
    Allows journalists to edit a rejected article and resubmit it.
    """
    ...
    if request.method == "POST" and form.is_valid():
        article = form.save(commit=False)
        article.resubmit()  # Custom model method to set status to 'submitted'
        article.save()
...
def approve_article(request, pk):
    ...
    article.approve()  # Custom model method
    article.save()
...
def article_list(request):
    """
    Display articles depending on the user's role:
    - Reader: approved articles
    - Journalist: own articles
    - Editor: all articles
    """
    user = request.user
    if user.role == "reader":
...
def article_detail(request, pk):
    """
    Displays the full article content. Role-based access:
...
# -------------------------------
# Search Articles by Query & Tag
# -------------------------------
def search_articles(request):
    """
    Allows users to search articles by title, content or tags.
    Also provides top tags for suggestions.
    """
...
# -------------------------------
# Editor: rejection article
# -------------------------------
@login_required
def reject_article(request, pk):
    """
    Allows editors to reject a submitted article and provide a rejection reason.
    """
    if request.user.role != "editor":
        messages.error(request, "Permission denied.")
        return redirect("home")
